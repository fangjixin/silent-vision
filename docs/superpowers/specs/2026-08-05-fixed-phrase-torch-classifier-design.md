...
It must return `UNKNOWN` when confidence or top-1 margin is insufficient. The
classifier is not presented as open-vocabulary lipreading or arbitrary speech
transcription.
...
- Safe rejection of unregistered or uncertain clips.
...
The builder creates deterministic `train.jsonl`, `validation.jsonl`, and
`inventory.json` artifacts from stable sample IDs. Every enabled phrase must have
examples in both splits. The inventory reports per-phrase counts, exclusions,
duplicates, intent mismatches, hashes, and split membership.

The current four-versus-five sample inventory is sufficient only to exercise the
pipeline. It is not sufficient for a reliable accuracy claim. The contest demo
target is at least 8-12 independent takes per phrase, with 2-3 held out for
validation, plus separate unrelated-phrase clips for rejection evaluation.

Unrelated clips have expected label `UNKNOWN` only in validation data. They are
not added as a learned phrase class.
...
the 256-value motion map. A learned projection maps those 512 visual values to the
temporal feature dimension consumed by the existing Conformer backend.
...
The front end and Conformer are one versioned Torch model used identically by
training and runtime inference. Its output size is dynamic and equals the number
of enabled phrase IDs in the checkpoint catalog. Feature configuration is stored
in the checkpoint. Training with one feature path and inferring with another is
forbidden.

Seeded training-only augmentation may apply small brightness, spatial-shift, and
temporal-jitter changes to training clips. Validation clips are never augmented.
...
An accepted phrase must satisfy both a minimum top-1 probability and a minimum
top-1/top-2 margin. Otherwise the decision is `UNKNOWN`, carries no phrase text,
and cannot execute an action.

Thresholds are command-line or configuration inputs and are saved with validation
evidence. They are not selected from the same clips used to train the classifier.
...
configuration, or an empty model state. A legacy intent-only checkpoint is
...
- A validator that emits phrase accuracy, mapped-intent accuracy, accepted
  precision, rejection rate, per-phrase counts, confusion matrix, thresholds,
  checkpoint hash, manifest hash, and ROCm device evidence.
...
- confidence and margin rejection with no executable route;
- phrase-oriented top-K output;
- WebSocket output from a synthetic Torch checkpoint; and
- validation-report schema.

Radeon verification separately proves HIP availability, GPU-only training,
checkpoint loading, one accepted known phrase, one rejected unrelated phrase, and
saved JSON evidence.
...
ROCm device evidence, and held-out validation output.
...
4. Uncertain or unrelated input returns `UNKNOWN` and cannot execute.
