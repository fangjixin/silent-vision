...
def test_later_sample_id_duplicate_is_rejected_even_if_first_record_is_excluded(tmp_path):
    ...
    (sample / "metadata.json").write_text(
        json.dumps(
            {"sampleId": "duplicate", "phrase": "你好，请帮我打开灯", "intent": "LIGHT_ON", "language": "zh"},
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
...
def test_inventory_includes_per_phrase_counts_and_split_membership(tmp_path):
    ...
    assert inventory["counts"]["byPhrase"] == {"zh_light_on_hello": 3, "zh_chat_meal": 0}
